...
(function (window) {
  ...
  var onHashChange = function (e) {
    // 忽略自动设置的跳转
    if (!isFirstLoad && /!!__t=[0-9]+$/.test(location.hash))
      return;
    ...
    var changeHash = function () {
      ...
      if (location.hash.indexOf('?') === -1)
        ...
        hash = hash.replace(/!!__t=[0-9]+$/img, '') + '&!!__t=' + timestamp;
      location.hash = hash;
    };
